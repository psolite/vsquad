...
import { getAllSquads, Player } from '../db'
import { pointsForEvent, playerMatches, EventType, POINTS } from './scoring'
import { scoreEmitter, GoalEvent, CardEvent } from './txodds/index'

// ── Player ID discovery (accumulates as stream events come in) ────────────────
...
function recordDiscovery(playerId: number, team: string, event: string) {
  if (!_discovered.has(playerId)) {
    _discovered.set(playerId, { txoddsId: playerId, team, events: [] })
  }
...
// In-memory store — keyed by walletAddress → playerId → PlayerLiveScore
const state: Record<string, Record<string, PlayerLiveScore>> = {}

// ── Live match scores (keyed by fixtureId) ────────────────────────────────────
...
function applyEvent(wallet: string, player: Player, event: EventType) {
  ...
  const pts   = pointsForEvent(player, event)
  entry.points += pts
...
function handleScoringEvent(
  ...
  for (const squad of squads) {
    for (const player of Object.values(squad.squad) as Player[]) {
      ...
      console.log(`[scoring] ${label}: ${player.name} (${player.country}) +${pointsForEvent(player, event)}pts → squad ${squad.squadName}`)
      matched = true
    }
  }
...
export function buildLeaderboard(): SquadLiveScore[] {
  ...
  const rows: SquadLiveScore[] = squads.map(squad => {
    ...
    // Include all 5 squad players, defaulting to 0 pts if no events yet
    const players = (Object.values(squad.squad) as Player[]).map(p => {
      ...
      return state[squad.walletAddress]?.[p.id] ?? {
        ...
        country: p.country, goals: 0, assists: 0,
        yellowCards: 0, redCards: 0, points: 0,
      }
