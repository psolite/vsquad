...
router.post('/:id/join', (req: Request, res: Response) => {
  ...
  // Must have a saved squad to join
  const squad = getSquad(walletAddress)
  if (!squad) return res.status(403).json({ error: 'You must save a squad before joining a tournament' })

  const t = joinTournament(req.params.id, walletAddress)
...
router.delete('/:id/leave', (req: Request, res: Response) => {
  ...
  const ok = leaveTournament(req.params.id, walletAddress)
