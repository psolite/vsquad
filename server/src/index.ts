...
app.listen(PORT, async () => {
  ...
  if (existingToken) {
    ...
    try {
      ...
      console.log(
        "[txodds] save to .env to skip auth next restart: TXODDS_API_TOKEN=" +
          apiToken,
      );
      await initTxodds(apiToken);
    } catch (err: any) {
      console.error(
        "[txodds] init failed — live scores unavailable:",
        err.message,
      );
    }
  } else {
    console.warn(
      "[txodds] set TXODDS_API_TOKEN or SERVER_WALLET_SECRET_KEY to enable live data",
    );
  }
});
