  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4" onClick={onClose}>
      ...
      <div className="relative z-10 w-full max-w-sm" style={{ background: '#0f1923', border: '1px solid rgba(255,255,255,0.08)', borderRadius: '20px', padding: '28px 24px 24px' }} onClick={(e) => e.stopPropagation()}>
        ...
        <button onClick={onClose} style={{ position: 'absolute', top: '14px', right: '14px', background: 'rgba(255,255,255,0.07)', border: 'none', cursor: 'pointer', width: '32px', height: '32px', borderRadius: '50%', color: 'rgba(255,255,255,0.7)', fontSize: '14px', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>✕</button>
        ...
        <div style={{ textAlign: 'center', marginBottom: '24px' }}>
          ...
          <p style={{ color: 'rgba(255,255,255,0.7)', fontSize: '13px', margin: 0 }}>{positionLabel[player.position]} · {player.country}</p>
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '10px', marginBottom: '20px' }}>
          {[{ value: player.goals, label: "Int'l Goals" }, { value: player.caps, label: 'Caps' }].map(({ value, label }) => (
            <div key={label} style={{ background: 'rgba(255,255,255,0.05)', borderRadius: '12px', padding: '14px 12px', textAlign: 'center' }}>
              ...
              <div style={{ color: 'rgba(255,255,255,0.7)', fontSize: '10px', textTransform: 'uppercase', letterSpacing: '0.1em', marginTop: '6px' }}>{label}</div>
            </div>
          ))}
        </div>
        ...
        {canAdd ? (
          ...
          <p style={{ textAlign: 'center', color: 'rgba(255,255,255,0.7)', fontSize: '13px' }}>Select a slot on the pitch first</p>
        )}
      </div>
    </div>
  )
