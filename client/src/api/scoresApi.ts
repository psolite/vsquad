...
export interface Fixture {
  ...
  statusCode:  string | null   // e.g. 'H1', 'HT', 'H2', 'F', 'FET', 'FPE'
  minute:      number | null
}
...
export const scoresApi = {
  ...
  matches: async (): Promise<MatchesResponse> => {
    const res = await fetch(`${BASE}/api/scores/matches`)
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    return res.json()
  },

  points: async (): Promise<Record<string, number>> => {
    const res = await fetch(`${BASE}/api/scores/points`)
    if (!res.ok) throw new Error(`HTTP ${res.status}`)
    return res.json()
  },
  ...
  subscribeToLive(handlers: {
    ...
    const es = new EventSource(`${BASE}/api/scores/live`)

    if (handlers.onGoal) {
      es.addEventListener('goal', (e) => {
        try { handlers.onGoal!(JSON.parse((e as MessageEvent).data)) } catch {}
      })
    }
    if (handlers.onLeaderboard) {
      es.addEventListener('leaderboard', (e) => {
        try { handlers.onLeaderboard!(JSON.parse((e as MessageEvent).data)) } catch {}
      })
    }
    if (handlers.onScoreUpdate) {
      es.addEventListener('score-update', (e) => {
        try { handlers.onScoreUpdate!(JSON.parse((e as MessageEvent).data)) } catch {}
      })
    }
